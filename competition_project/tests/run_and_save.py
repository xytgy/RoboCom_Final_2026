...
print_and_log(f"原始文本长度: {len(long_text)} 字\n")
for size in [100, 200]:
    chunks = split_text(long_text, chunk_size=size, overlap=20)
    print_and_log(f"切分大小: {size} → {len(chunks)} 个片段")
    for i, c in enumerate(chunks[:2], 1):
        print_and_log(f"  片段 {i}: {c.strip()[:50]}...")
    print_and_log("")
print_and_log("✓ 切分测试通过!\n")

print_and_log("="*60)
print_and_log("测试 2: RAG 检索")
print_and_log("="*60)
from rag.vector_store import VectorStore

store = VectorStore()
print_and_log("✓ 向量模型加载成功\n")
...
for t in test_texts:
    store.add(t)
print_and_log(f"✓ 添加了 {len(test_texts)} 条文本\n")

query = "编程语言"
results = store.search(query, k=2)
print_and_log(f"查询 '{query}' 结果:")
